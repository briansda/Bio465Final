...
ComBat <- function(dat, saminfo, type='txt', write=T, covariates='all', par.prior=T, filter=F, skip=0, prior.plots=F){
	...
	if (any(is.na(tmp)))
    {
      print('ERROR: Sample Information File and Data Array Names are not the same!')
      print('Samples that are not overlapping:')
      print(sort(setdiff(unique(union(colnames(dat), saminfo[,1])), intersect(colnames(dat), saminfo[,1]))))
      return(NULL)
    }
...
data = read.table(inFilePath, sep="\t", header=TRUE, row.names=1, quote="\"", check.names=FALSE)

batchData = read.table(batchFilePath, sep="\t", stringsAsFactors=F, row.names=1, header=T, check.names=F)

overlappingNames = intersect(colnames(data), rownames(batchData))
nonOverlappingNames = sort(setdiff(unique(union(colnames(data), rownames(batchData))), intersect(colnames(data), rownames(batchData))))

if (length(overlappingNames) == 0)
  stop('No samples overlapped between the expression file and the batch file.')

if (length(nonOverlappingNames) > 0)
{
  print('The following samples did not overlap between the expression file and the batch file, so they will be excluded.')
  print(nonOverlappingNames)
}

data = data[,overlappingNames]
batchData = batchData[overlappingNames,]

print("Running combat")
adjustedData = ComBat(data, batchData)
write.table(adjustedData, file=outFilePath, quote=FALSE, col.names=NA, row.names=TRUE, sep="\t")

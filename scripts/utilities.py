...
def fisherExactTestTwoTail(x):
    return FishersExactTest(x).two_tail_p()

def fisherExactTestRightTail(x):
    return FishersExactTest(x).right_tail_p()

def fisherExactTestLeftTail(x):
    return FishersExactTest(x).left_tail_p()
...
# See http://www.physicsforums.com/showthread.php?t=420714
#def calculateOddsRatio2By2(a, b, c, d):
#    numerator = math.factorial(a+b) * math.factorial(c+d) * math.factorial(a+c) * math.factorial(b+d)
#    denominator = math.factorial(a) * math.factorial(b) * math.factorial(c) * math.factorial(d) * math.factorial(sum([a, b, c, d]))
#
